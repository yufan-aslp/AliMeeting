...
train_overlap_merge_speaker_dir=data/local//aishell4_${tgt}_overlap_merge_speaker
...
mkdir -p $train_overlap_merge_speaker_dir
mkdir -p $tmp_dir
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then 
    log "stage 6: train_overlap_merge_speaker_dir preocess"
    cp $train_nooverlap_dir/* $train_overlap_merge_speaker_dir/
    python local/aishell4_process_textgrid_speaker.py --path $train_overlap_merge_speaker_dir  \
        --overlap_length 0.8 --max_length 7
    cat $train_overlap_merge_speaker_dir/text_all | local/text_normalize.pl | local/text_format.pl | sort -u > $train_overlap_merge_speaker_dir/text
    utils/filter_scp.pl -f 1 $train_overlap_merge_speaker_dir/text $train_overlap_merge_speaker_dir/utt2spk_all | sort -u > $train_overlap_merge_speaker_dir/utt2spk
    utils/utt2spk_to_spk2utt.pl $train_overlap_merge_speaker_dir/utt2spk > $train_overlap_merge_speaker_dir/spk2utt
    utils/filter_scp.pl -f 1 $train_overlap_merge_speaker_dir/text $train_overlap_merge_speaker_dir/segments_all | sort -u > $train_overlap_merge_speaker_dir/segments
    utils/filter_scp.pl -f 1 $train_overlap_merge_speaker_dir/text $train_overlap_merge_speaker_dir/utt2spk_new | sort -u > $train_overlap_merge_speaker_dir/utt2spk_merge
fi

if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
    log "stage 7: finali data process"
    ...
    utils/copy_data_dir.sh --utt-prefix aishell4overlapmergespeaker- --spk-prefix aishell4overlapmergespeaker-  \
        $train_overlap_merge_speaker_dir data/aishell4_${tgt}_overlap_merge_speaker
    cp $train_overlap_merge_speaker_dir/utt2spk_merge data/aishell4_${tgt}_overlap_merge_speaker
    # remove space in text
    #for x in aishell4_${tgt}_nooverlap aishell4_${tgt}_overlap aishell4_${tgt}_overlap_merge; do
    for x in aishell4_${tgt}_overlap_merge_speaker; do
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
    log "stage 8: process mars one channel"
    #for x in aishell4_${tgt}_nooverlap aishell4_${tgt}_overlap aishell4_${tgt}_overlap_merge; do
    for x in aishell4_${tgt}_overlap_merge_speaker; do
...
if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
    merge_speaker_dir=data/aishell4_${tgt}_overlap_merge_speaker_onechannel/
    cp $merge_speaker_dir/text $merge_speaker_dir/text_tmp1
    cp $merge_speaker_dir/utt2spk_merge $merge_speaker_dir/utt2spk_merge_tmp1
    sed -e 's/^/aishell4overlapmergespeaker-/g' $merge_speaker_dir/utt2spk_merge_tmp1 >$merge_speaker_dir/utt2spk_merge_tmp2 
    sed -e 's/src/$/g' $merge_speaker_dir/utt2spk_merge_tmp2 > $merge_speaker_dir/utt2spk_merge_process
    sed -e 's/SRC/$/g' $merge_speaker_dir/text_tmp1 > $merge_speaker_dir/text_process
    python local/aishell4_process_empty_text_speaker.py --path $merge_speaker_dir
    sed -e 's/ SRC/ /' $merge_speaker_dir/text_new > $merge_speaker_dir/text_new_tmp1
    sed -e 's/ SRC/ /' $merge_speaker_dir/utt2spk_merge_new >$merge_speaker_dir/utt2spk_merge_new_tmp1
    sed -e 's/SRC/$/g' $merge_speaker_dir/text_new_tmp1 > $merge_speaker_dir/text_final
    sed -e 's/SRC/$/g' $merge_speaker_dir/utt2spk_merge_new_tmp1 >$merge_speaker_dir/utt2spk_merge_final
